...
import { ModuleSubscribeToCoinInfoHandle } from '@frontend/common/src/modules/handlers/balanceMonitor/ModuleSubscribeToCoinInfoHandle';
...
const createFail = FailFactory('CoinInfo');
const descFactory = ValueDescriptorFactory<
    ReadonlyMap<TCoinId, TFullInfoByCoin>,
    TFail<'[CoinInfo]: UNKNOWN'>
>();

export type TCoinInfoDescriptor = ExtractValueDescriptor<typeof descFactory>;

export const ModuleCoinInfo = ModuleFactory((ctx: TContextRef) => {
    let subscribeTraceId: TraceId | undefined = undefined;

    const getCoinInfo$ = dedobs(
        (traceId: TraceId): Observable<TCoinInfoDescriptor> => {
            const subscribeToCoinInfoHandle = ModuleSubscribeToCoinInfoHandle(ctx);
            ...
            return currentSocketUrl$.pipe(
                ...
                distinctUntilChanged(),
                switchMap((url) =>
                    subscribeToCoinInfoHandle(url, {}, { traceId }).pipe(
        ...
        },
        {
            normalize: ([traceId]) => {
                ...
                if (subscribeTraceId !== undefined && subscribeTraceId !== traceId) {
                    logger.info(
                        `ModuleGetCoinInfo: traceId ${traceId} reuse observable with traceId ${subscribeTraceId}`,
                    );
                }

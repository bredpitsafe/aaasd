...
import { ModuleSubscribeToTransferHistoryHandle } from '@frontend/common/src/modules/handlers/balanceMonitor/ModuleSubscribeToTransferHistoryHandle';
...
const createFail = FailFactory('TransfersHistory');
const descFactory = ValueDescriptorFactory<
    TTransferHistoryItem[],
    TFail<'[TransfersHistory]: UNKNOWN'>
>();

export type TTransfersHistoryDescriptor = ExtractValueDescriptor<typeof descFactory>;

export const ModuleTransfersHistory = ModuleFactory((ctx: TContextRef) => {
    let subscribeTraceId: TraceId | undefined = undefined;

    const getTransfersHistory$ = dedobs(
        (traceId: TraceId): Observable<TTransfersHistoryDescriptor> => {
            const subscribeToTransferHistoryHandle = ModuleSubscribeToTransferHistoryHandle(ctx);
            ...
            return currentSocketUrl$.pipe(
                ...
                distinctUntilChanged(),
                switchMap((url) =>
                    subscribeToTransferHistoryHandle(url, {}, { traceId }).pipe(
                        ...
                        scan((acc, payloads) => {
                            ...
                            return acc;
                        }, new UnifierWithCompositeHash<TTransferHistoryItem>('clientId')),
        ...
        },
        {
            normalize: ([traceId]) => {
                ...
                if (subscribeTraceId !== undefined && subscribeTraceId !== traceId) {
                    logger.info(
                        `ModuleGetTransfersHistory: traceId ${traceId} reuse observable with traceId ${subscribeTraceId}`,
                    );
                }

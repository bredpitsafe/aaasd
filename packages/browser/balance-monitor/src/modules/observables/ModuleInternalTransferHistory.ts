...
import { ModuleSubscribeToInternalTransferHistoryHandle } from '@frontend/common/src/modules/handlers/balanceMonitor/ModuleSubscribeToInternalTransferHistoryHandle';
...
const createFail = FailFactory('InternalTransferHistory');
const descFactory = ValueDescriptorFactory<
    TInternalTransfer[],
    TFail<'[InternalTransferHistory]: UNKNOWN'>
>();

export type TInternalTransferHistoryDescriptor = ExtractValueDescriptor<typeof descFactory>;

export const ModuleInternalTransferHistory = ModuleFactory((ctx: TContextRef) => {
    let subscribeTraceId: TraceId | undefined = undefined;

    const getInternalTransferHistory$ = dedobs(
        (traceId: TraceId): Observable<TInternalTransferHistoryDescriptor> => {
            const subscribeToInternalTransferHistoryHandle =
                ModuleSubscribeToInternalTransferHistoryHandle(ctx);
            ...
            return currentSocketUrl$.pipe(
                ...
                distinctUntilChanged(),
                switchMap((url) =>
                    subscribeToInternalTransferHistoryHandle(url, {}, { traceId }).pipe(
        ...
        },
        {
            normalize: ([traceId]) => {
                ...
                if (subscribeTraceId !== undefined && subscribeTraceId !== traceId) {
                    logger.info(
                        `ModuleInternalTransferHistory: traceId ${traceId} reuse observable with traceId ${subscribeTraceId}`,
                    );
                }

...
import { TIndicatorKey } from '@frontend/common/src/modules/actions/indicators/defs';
...
import {
    ...
    TSingleValueSandbox,
} from '@frontend/common/src/utils/Sandboxes/numberConversion';
...
import { ChartLegendsView, TChartLegendsView } from '../../ChartLegends/view';
...
export function ConnectedChartLegends(props: TConnectedChartLegendsView): ReactElement {
    ...
    const mapChartIdToIndicatorsQuery = useMemo(
        () =>
            charts.reduce(
                (acc, chart) => {
                    const indicatorName = Promql.tryParseQuery(chart.query)?.labels?.name;

                    acc[chart.id] = {
                        url: chart.url ?? props.url,
                        btRuns: isNil(props.backtestingId) ? undefined : [props.backtestingId],

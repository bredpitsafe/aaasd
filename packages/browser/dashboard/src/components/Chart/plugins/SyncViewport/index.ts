...
import { receiveFromTabs, sendToTabs } from '@frontend/common/src/utils/Rx/syncBetweenTabs';
...
export class SyncViewportPlugin implements IPlugin {
    ...
    private readonly readyForSyncHosts = new Set<TimeseriesCharter>();
    ...
    readyForSync(host: TimeseriesCharter): void {
        ...
        if (this.seedUpdateData !== undefined && this.readyForSyncHosts.size === 1) {
    ...
    disconnect(host: TimeseriesCharter): void {
        this.readyForSyncHosts.delete(host);
    ...
    private limitHorizontalScale(event: TMeasureZoomHorizontalEvent, host: TimeseriesCharter) {
        ...
        if (isNil(newScaleX)) {
            console.error(`Trying to set horizontal scale ${event.scale} which is not supported`);
            return;
        }

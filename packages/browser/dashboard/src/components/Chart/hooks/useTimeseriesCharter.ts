...
import type {
    ...
    Someseconds,
    TTimeZoneInfo,
} from '@frontend/common/src/types/time';
...
export function useTimeseriesCharter(props: TProps) {
    ...
    const getChartClosestPoints = useFunction(() => {
        ...
        return charter.getClosestPoints().map(({ id, point, nonNaNPoint, canvasPoint }) => ({
            id,
            point: {
                x: point.x + charter.getServerTimeIncrement() - timeZoneShift,
                y: point.y,
            },
            nonNaNPoint:
                nonNaNPoint === undefined
                    ? undefined
                    : {
                          x: nonNaNPoint.x + charter.getServerTimeIncrement() - timeZoneShift,
                          y: nonNaNPoint.y,
                      },
            canvasPoint,
        }));
    ...
    return { charter, getChartClosestPoints, getMouseCoords };
}

...
globalStyle(`${cnPermissionsGrid} .ag-header-row.ag-header-row-column`, {
    display: 'none',
});

globalStyle(`${cnPermissionsGrid} .ag-header-row:not(:first-child) .ag-header-cell:not(#\\9)`, {
    borderTop: 0,
});
...
export const cnAddUserFormContainer = style({
    display: 'flex',
    flexFlow: 'row',
});

export const cnAddUserFormInput = style({
    flex: '1 1 auto',
});

export const cnAddUserFormSubmit = style({
    flex: '0 0 auto',
});

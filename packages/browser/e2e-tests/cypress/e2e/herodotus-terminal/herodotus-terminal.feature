Feature: e2e tests: "Herodotus Terminal" page test suit. I as User checks the functionality of "Herodotus Trades" page
  ...
  Scenario: Checking "Started" and "Paused" a task via the button
    ...
    And user deletes all archive tasks
    And user archive active tasks
    And user selects the task by id "20668" and selects in the context menu of "Clone" in the "Active Tasks" tab
    ...
    When user not sees "paused" status in the table on the "Active Tasks" tab
  ...
  Scenario: Checking clone a task via the context menu in ths "Active Tasks" tab
    ...
    And user selects the task by id "20668" and selects in the context menu of "Clone" in the "Active Tasks" tab
    And user confirms the creation of the task
    When user sees the successful "Generic command" messages
  ...
  Scenario: Checking fill form from this task via the context menu in the "Active Tasks" tab
    ...
    And user deletes all archive tasks
    And user archive active tasks
    And user selects the task by id "20668" and selects in the context menu of "Fill form from this task" in the "Active Tasks" tab
  ...
  Scenario Outline: Checking the row download by "<NameButton>" button in the "<NameTab>" tab
    ...
    Examples:
      ...
      | CSV        | 1 table rows copied to clipboard as CSV  | 20668 | Active Tasks | herodotus-terminal/archived-task.csv  |
      | JSON       | 1 table rows copied to clipboard as JSON | 20668 | Active Tasks | herodotus-terminal/archived-task.json |
  ...
  Scenario Outline: Checking the row download by "<NameExport>" button in the "<NameTab>" tab
    ...
    Examples:
      ...
      | CSV Export    | 20668 | Active Tasks   |
      | Excel Export  | 20668 | Active Tasks   |
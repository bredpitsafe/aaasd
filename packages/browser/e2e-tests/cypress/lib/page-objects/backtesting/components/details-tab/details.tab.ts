...
class DetailsTab {
    ...
    readonly deleteScoreIndicatorInput = new Text(
        EBacktestingSelectors.DeleteScoreIndicatorInput,
        false,
    );
    ...
    clearTaskForm() {
        ...
        cy.get(EDetailsTabSelectors.DetailsTab).then(($body) => {
            if ($body.find(EBacktestingSelectors.DeleteScoreIndicatorInput).length > 0) {
    ...
    setTaskForm() {
        ...
        cy.get(testSelector(EBacktestingSelectors.ScoreIndicatorInput)).type(
    ...
    setIndicator(nameIndicator: string) {
        ...
        cy.get(testSelector(EBacktestingSelectors.ScoreIndicatorInput)).type(nameIndicator);
    }

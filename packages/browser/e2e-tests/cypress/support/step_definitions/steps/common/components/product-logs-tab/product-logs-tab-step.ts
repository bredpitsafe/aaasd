...
Given(/user adds "Level" filters And checks the addition "Level" filters/, () => {
    productLogsTab.levelSelector.selectByText('Info');
    productLogsLevelFilter.infoFilterText.checkVisible();
    productLogsTab.levelSelector.clear();
    productLogsTab.levelSelector.selectByText('Warn');
    productLogsLevelFilter.warnFilterText.checkVisible();
    productLogsTab.levelSelector.clear();
    productLogsTab.levelSelector.selectByText('Error');
...
Given(`user selects {string} from the "Level" filter`, (nameLevel: string) => {
    productLogsTab.levelSelector.selectByText(nameLevel);
...
Given(`user types {string} data in the calendar`, (data: string) => {
    productLogsTab.sinceButton.clickClearAndType(data);
...
Given(
    ...
    (inputType: string, nameInput: string) => {
        switch (nameInput) {
            case 'Message':
                productLogsTab.includeMessageInput.selectByText(inputType);
                break;
            case 'Actor Key':
                productLogsTab.includeActorKeyInput.selectByText(inputType);
                break;
            case 'Actor Group':
                productLogsTab.includeActorGroupInput.selectByText(inputType);
                break;
        }
        productLogsTab.applyButtonClick();
        customWait(1);
...
Given(
    ...
    (inputType: string, nameInput: string) => {
        switch (nameInput) {
            case 'Message':
                productLogsTab.excludeMessageInput.selectByText(inputType);
                break;
            case 'Actor Key':
                productLogsTab.excludeActorKeyInput.selectByText(inputType);
                break;
            case 'Actor Group':
                productLogsTab.excludeActorGroupInput.selectByText(inputType);
                break;
        }
        productLogsTab.applyButtonClick();
        customWait(1);
...
Given(`user scrolls {string} the "Product Logs" table`, (value: string) => {
    switch (value) {
        case 'down':

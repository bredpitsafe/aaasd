...
Given(`user selects the {string} of the "Disabled" selector`, (value: string) => {
    addTransferBlockingRuleTab.disabledSelector.selectByText(value);
});
...
Given(`user sees the "Days" and "Hours" selector in the "Add Transfer Blocking Rule" form`, () => {
    ...
    addTransferBlockingRuleTab.periodSelector.clickByText('Hours');
...
Given(`user sets the date {string} in the "End time" calendar`, (data: string) => {
    addTransferBlockingRuleTab.endPeriodInput.clickAndType(data);
});
...
Given(
    ...
    (nameUser: string) => {
        tableFilter.nameInput.typeAndEnter(nameUser);
    ...
    (nameValue: string, nameRow: string) => {
        switch (nameRow) {
            case 'status':
                ...
                break;
            case 'coin':
                transferBlockingRulesTableRow.checkContainCoin(nameValue);
                break;
            case 'userName':

...
Given(`user types {string} in the {string} input`, (value: string, nameInput: string) => {
    ...
    switch (nameInput) {
        ...
        case 'Virtual Account Name':
            ...
            break;
        case 'Real Accounts':
            virtualAccountsNewAccountModal.realAccountsSelect.selectByText(value);
            break;
    }
});
...
Given(`user edits "Credentials" values:`, (dataTable: DataTable) => {
    ...
    cy.get('@userName').then((object) => {
        ...
        tableFilter.nameInput.typeAndEnter(userName);
        realAccountsTableRow.doubleClickRowByName(userName);
    });
    dataTable.hashes().forEach((element) => {
        realAccountsNewAccountModal.credentialsNameInput.clearAndType(element.name);
        realAccountsNewAccountModal.credentialsKeyInput.clearAndType(element.key);
        realAccountsNewAccountModal.credentialsSecretEditInput.type(element.secret);
        realAccountsNewAccountModal.credentialsPassphraseEditInput.type(element.passphrase);
    });
});

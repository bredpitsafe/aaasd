...
import { ISO, Milliseconds } from '../types/time';
...
import { getNowISO, milliseconds2iso, toISO } from './time';
import { compareDates, isClampedDates, isEqualDates, maxDate, minDate } from './timeCompare';
import { TimeseriesSliceManager } from './TimeseriesSliceManager';
import { Slice } from './TimeseriesSliceManager/Slice';
    ...
    private fillEmptySlices(
        ...
        return this.fetchItems(
            ...
            mapValueDescriptor(({ value: items }) => {
                checkItemsInfinityLoop(items, restCount, this.getTime);

                const isHistoryRequest = compareDates(rightBound, this.getTimeNow()) <= 0;
                const _leftBound = items.length >= restCount ? this.getTime(items[0]) : leftBound;
                const _leftBoundInclude = items.length >= restCount ? true : leftBoundInclude;
...
function checkItemsInfinityLoop<T>(items: T[], length: number, getTime: (item: T) => ISO) {
    if (items.length === length && getTime(items[0]) === getTime(items[items.length - 1])) {

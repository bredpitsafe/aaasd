...
export class SnapshotStore<T> {
    ...
    upsertSnapshotItem(value: T) {
        this.updateItemIn(this.snapshotMap, this.getKey(value), value);
        this.invalidateHistoryItems();
    }

    upsertSnapshotItems(values: Array<T>) {
        for (let i = 0; i < values.length; i++) {
            this.updateItemIn(this.snapshotMap, this.getKey(values[i]), values[i]);
        }

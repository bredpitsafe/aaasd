...
import { ReconnectOnTimeout } from '../../lib/BFFSocket/plugins/ReconnectOnTimeout';
...
const getCommonPlugins = (
    ...
    return [
        ...
        new ReconnectOnTimeout({ delay: NO_MESSAGES_DELAY }),
...
const createSocketBank = (
    ...
    createBank({
        ...
        createValue: (key, { name, url }) => {
            logger.info(`[SocketBank] Create sockets, url: ${url}`);

            const getPlugins = (name: TDataSourceName) => getCommonPlugins(ctx, name, url);
            const urlInstance = new URL(url);

            return new BFFSocket(urlInstance, undefined, getPlugins(`${contextName} ${name}`));
        },

        onRemove: (url, socket) => {
            logger.info(`[SocketBank] Delete sockets, url: ${url}`);
            socket.destroy();
        },

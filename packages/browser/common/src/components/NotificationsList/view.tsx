...
import { ENotificationsType, TNotification } from '../../modules/notifications/def';
...
function NotificationGroup({
    ...
    const messages = items.map((item) => (
        <CommonErrorView key={item.timestamp} style={{ marginBottom: '8px' }} {...item} />
    ));
    const cbClose = useFunction(() => items.forEach(({ id }) => onClose?.(id)));

    return (
        <Alert
            {...lastItem.listSettings}
            key={lastItem.timestamp}
            className={cnAlert}
            type={lastItem.type}
            message={messages}
            closable={lastItem.listSettings?.closable ?? true}
            onClose={cbClose}
        />
    );
}

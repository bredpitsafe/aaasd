...
import { TPart, TPartClosestAbsPoint, TPartId, TSeriesId } from '../../../lib/Parts/def';
import {
    getFirstAbsPoint,
    getLastAbsPoint,
        ...
        const suitableRightPart =
            suitableLeftPart === undefined && part.size === 0
                ? Enumerable.from(partTraverseToRightSide(getParts, part))
                      .takeWhile(
                          (part) =>
                              !isUndefined(part.absLeftPoint) ||
                              (part.unresolved === false && part.size === 0),
                      )
                      .where((part) => !isUndefined(part.absLeftPoint))
                      .firstOrDefault(undefined)
                : undefined;
        ...
        const suitableLeftPart =
            suitableRightPart === undefined && part.size === 0
                ? Enumerable.from(partTraverseToRightSide(getParts, part))
                      .takeWhile(
                          (part) =>
                              !isUndefined(part.absRightPoint) ||
                              (part.size === 0 && part.unresolved === false),
                      )
                      .where((part) => !isUndefined(part.absRightPoint))
                      .firstOrDefault(undefined)
                : undefined;
...
function* partTraverseToLeftSide(
    ...
    while (true) {
        ...
        if (parts.length > 1) {
            console.error('[Charter] In one place more than one part');
            break;
        }
...
function* partTraverseToRightSide(
    ...
    while (true) {
        ...
        if (parts.length > 1) {
            console.error('In one place more than one part');
            break;
        }

import { TPoint } from '@frontend/common/src/types/shape';
...
export class MouseClosestPointsController extends Container {
    ...
    constructor(public ctx: IContext) {
        ...
        fromEvent(targetView, 'mouseenter', { passive: true })
            .pipe(
                switchMap(() => {
                    ...
                    return tickerController.getTicker$().pipe(
                        map(() => {
                            const point = mouseController.getCanvasMouseCoords();
                        ...
                        takeUntil(
                            fromEvent(ctx.targetView, 'mouseleave', {
                                passive: true,
                            }),
                        ),
                        finalize(() => this.clearClosestPoints()),
                    );
                }),
                takeUntil(this.destroyer$),
            )
            .subscribe((point) => this.computeClosestPoints(point));
    ...
    private findLeftClosestPoints(point: TPoint): TClosestPointData[] {
    ...
    private findLeftClosestSinglePoint(mousePoint: TPoint): TClosestPointData[] {
    ...
    private findLeftClosestSlicePoints = (mousePoint: TPoint): TClosestPointData[] => {
        const { mouseController, partsController, chartsController, virtualViewportController } =
            this.ctx;
        ...
        const mapChartToOptionalPointData = ({
            id,
            yAxis,
        }: TChartProps): TClosestPointData | undefined => {
        ...
        return chartsController
            ...
            .filter((data): data is TClosestPointData => data !== undefined);
    };
}

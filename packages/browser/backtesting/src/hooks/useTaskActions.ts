...
export const useTaskActions = (): TUseTaskActionsReturnType => {
    ...
    const stopTask = useNotifiedObservableFunction((id: TBacktestingTask['id']) => {
    ...
    const runAgainTask = useNotifiedObservableFunction((id: TBacktestingTask['id']) => {
        ...
        return currentSocketUrl$.pipe(
            ...
            switchMap((url) =>
                forkJoin([
                    ...
                    getBacktestingTaskConfigs(id, traceId).pipe(
                        extractSyncedValueFromValueDescriptor(),
                    ),
                ]).pipe(
                    ...
                    extractSyncedValueFromValueDescriptor(),
                    filter(
                        (result): result is TBacktestingTask['id'] => typeof result === 'number',
                    ),
    ...
    const cloneTask = useNotifiedObservableFunction((id: TBacktestingTask['id']) => {
        const traceId = generateTraceId();
        ...
        return currentSocketUrl$.pipe(
            ...
            switchMap((url) =>
                forkJoin([
                    ...
                    getBacktestingTaskConfigs(id, traceId).pipe(first(isSyncedValueDescriptor)),
                ]).pipe(
                    switchMap(
                        ([{ value: task }, { value: config }]): Observable<TFormBacktestingTask> =>
                            of({
                                id: task.id,
                                name: task.name,
                                description: task.description,
                                ...
                                scoreIndicatorsList: indicatorToIndicatorsList(task.scoreIndicator),
                            }),
                    ),
                ),
            ),
    ...
    const deleteTask = useNotifiedObservableFunction((id: TBacktestingTask['id']) => {

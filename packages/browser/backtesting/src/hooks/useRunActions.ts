...
import { TBacktestingRun, TBacktestingRunId } from '@frontend/common/src/types/domain/backtestings';
...
export const useRunActions = (): TUseRunActionsReturnType => {
    ...
    const pauseRun = useNotifiedObservableFunction((btRunNo: TBacktestingRun['btRunNo']) => {
    ...
    const resumeRun = useNotifiedObservableFunction((btRunNo: TBacktestingRun['btRunNo']) => {
        return currentSocketUrl$.pipe(
            first((target): target is TSocketURL => target !== undefined),

...
import type { TUpdateBacktestingTaskParams } from '@frontend/common/src/handlers/backtesting/updateBacktestingTaskHandle';
...
export function useConnectedScoreIndicatorsList(traceId: TraceId): {
    ...
    updateScoreIndicatorsListInTask: (scoreIndicator: string[]) => Promise<unknown>;
    ...
    const updateScoreIndicatorsListInTask = useNotifiedObservableFunction(
        (scoreIndicatorsList: string[]) => {
            ...
            const newTask: TUpdateBacktestingTaskParams = {
                ...value,

...
import { ModuleSubscribeToIndicatorsSnapshot } from '@frontend/common/src/modules/actions/indicators/snapshot';
...
export const ModuleSubscribeToIndicatorsMap = ModuleFactory((ctx) => {
    const subscribeToIndicatorsSnapshot = ModuleSubscribeToIndicatorsSnapshot(ctx);

    return dedobs(
        (
            url: TSocketURL,
            indicatorNames: TIndicator['name'] | TIndicator['name'][],
            ...
            return subscribeToIndicatorsSnapshot(
                {
                    url,
                    names: Array.isArray(indicatorNames) ? indicatorNames : [indicatorNames],
        ...
        },
        {
            ...
            resetDelay: 0,
            normalize: ([url, name, backtestingRunIds]) =>
                shallowHash(url, name, ...backtestingRunIds),
        },
    );
});

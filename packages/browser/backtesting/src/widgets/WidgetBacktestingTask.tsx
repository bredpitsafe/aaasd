...
import type { TValidFormBacktestingTask } from '../components/Forms/FormCreateBacktestingTask/defs';
import {
...
export function WidgetBacktestingTask(props: TWithClassname) {
    ...
    const cbUpdateTask = useFunction(async (task: TValidFormBacktestingTask) => {
        if (isNil(socketUrl) || isUndefined(taskId)) {
            return;
        }

        await firstValueFrom(
            updateBacktestingTask(
                {
                    target: socketUrl,
                    id: taskId,
                    name: task.name,
    ...
    return (
        <div className={cn(cnRelative, props.className)}>
            <ValueDescriptorsBadge descriptors={[taskWithStatus]} />
            {!isNil(formTask) && (

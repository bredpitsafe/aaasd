...
import { useConvertRate } from '@frontend/common/src/components/hooks/useConvertRates';
...
import { THerodotusReferenceCurrencyReturnType } from '@frontend/common/src/modules/actions/ModuleGetHerodotusReferenceCurrency';
...
import { THerodotusData } from '../hooks/useHerodotusData';
...
import { cnForm } from './FormHerodotusTask.css';
...
type TFormHerodotusTaskProps = TWithClassname & {
    ...
    referenceCurrencyDesc: THerodotusReferenceCurrencyReturnType;
...
export function FormHerodotusTask(props: TFormHerodotusTaskProps): ReactElement {
    ...
    const getSchema = useMemo(
        () =>
            memoize((type: EHerodotusTaskType) => {
                const isBuy = type === EHerodotusTaskType.Buy;
                const isSell = type === EHerodotusTaskType.Sell;
                const isBuySell = type === EHerodotusTaskType.BuySell;
    ...
    const cbSubmit = useFunction(async (fields: THerodotusTaskFormData) => {
        const isBuy = fields.type === EHerodotusTaskType.Buy;
        const isSell = fields.type === EHerodotusTaskType.Sell;
        const isBuySell = fields.type === EHerodotusTaskType.BuySell;
        const withBuy = isBuySell || isBuy;
        const withSell = isBuySell || isSell;
...
function FormHerodotusTaskRenderer(
    ...
    const convertRatesMap = useConvertRate(values.assetName);
    ...
    const isBuySell = values.type === EHerodotusTaskType.BuySell;
    const withBuy = isBuySell || values.type === EHerodotusTaskType.Buy;
    const withSell = isBuySell || values.type === EHerodotusTaskType.Sell;
    const roles = isBuySell ? ALL_ROLES : ONLY_QUOTE_ROLES;

    const oneWayInstruments = isBuySell
        ? undefined
        : values.type === EHerodotusTaskType.Buy
    ...
    const canUseCurrencyTypeQuote =
        oneWayInstruments !== undefined
            ...
            : false;
    const referenceCurrency = isSyncDesc(referenceCurrencyDesc)
            ...
            <Row gutter={[8, 16]}>
                ...
                <Col span={4}>
                    <FormikForm.Item label="Task Type" name="type">
                        ...
                        >
                            <FormikSelect.Option
                                key={EHerodotusTaskType.Buy}
                                value={EHerodotusTaskType.Buy}
                            >
                                Buy
                            </FormikSelect.Option>
                            <FormikSelect.Option
                                key={EHerodotusTaskType.Sell}
                                value={EHerodotusTaskType.Sell}
                            >
                                Sell
                            </FormikSelect.Option>
                            <FormikSelect.Option
                                key={EHerodotusTaskType.BuySell}
                                value={EHerodotusTaskType.BuySell}
                            >
                                BuySell
                            </FormikSelect.Option>
                        </FormikSelect>
...
function MinMaxLabel(props: TMinMaxLabelProps): ReactElement {
    ...
    const label =
        (type === EHerodotusTaskType.Buy ? 'Max' : 'Min') +

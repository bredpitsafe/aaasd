import { TUnsubscribe } from '@frontend/common/src/handlers/def';
import { BFFSocket } from '@frontend/common/src/lib/BFFSocket/BFFSocket';
import { TSendEnvelope } from '@frontend/common/src/lib/BFFSocket/def';
...
export const createHandler = (): TStreamHandler => {
    return <TStreamHandler>((url: TSocketURL, _sender, _options) => {
        ...
        return waitSocket$(socket, options).pipe(
            ...
            map((v) => v.value),
            options.waitForResponse ? timeoutOperator(request, options) : identity,

...
import { withAuthMetadata } from '../../transport/grpc/client/auth.ts';
...
export const convertRatesRpcRoutes = createRpcRoutes<ETradingDataProviderRouteName>({
    [ETradingDataProviderRouteName.SubscribeToConvertRates]: {
        ...
        handler(req, ctx) {
            return ctx
                .getGrpcClient(EGrpcClientName.ConvertRatesV1, req.payload.requestStage)
                .subscribeToConvertRates(
                    subscribeToConvertRatesTransformers.fromRequestToGrpc(req),
                    withAuthMetadata(ctx),
                )
                .pipe(
        ...
        handler(req, ctx) {
            return ctx
                ...
                .fetchConvertRatesLog(
                    fetchConvertRatesLogTransformers.fromRequestToGrpc(req),
                    withAuthMetadata(ctx),
                )
        ...
        handler(req, ctx) {
            return ctx
                ...
                .fetchConvertRatesSnapshot(
                    fetchConvertRatesSnapshotTransformers.fromRequestToGrpc(req),
                    withAuthMetadata(ctx),
                )
                .pipe(
                    map(({ data }) =>
    ...
    [ETradingDataProviderRouteName.SubscribeToStmBalances]: {
        ...
        handler(req, ctx) {
            return ctx
                .getGrpcClient(EGrpcClientName.StmV1, req.payload.requestStage)
                .subscribeToStmBalances(
                    subscribeToStmBalancesTransformers.fromRequestToGrpc(req),
                    withAuthMetadata(ctx),
                )
                .pipe(
    ...
    [ETradingDataProviderRouteName.SubscribeToStmPositions]: {
        ...
        handler(req, ctx) {
            return ctx
                .getGrpcClient(EGrpcClientName.StmV1, req.payload.requestStage)
                .subscribeToStmBalances(
                    subscribeToStmPositionsTransformers.fromRequestToGrpc(req),
                    withAuthMetadata(ctx),
                )
                .pipe(
        ...
        handler(req, ctx) {
            return ctx
                ...
                .fetchStmBalancesSnapshot(
                    fetchStmBalancesSnapshotTransformers.fromRequestToGrpc(req),
                    withAuthMetadata(ctx),
                )
                .pipe(
                    map(({ data }) =>
    ...
    [ETradingDataProviderRouteName.FetchStmPositionsSnapshot]: {
        ...
        handler(req, ctx) {
            return ctx
                .getGrpcClient(EGrpcClientName.StmV1, req.payload.requestStage)
                .fetchStmBalancesSnapshot(
                    fetchStmPositionsSnapshotTransformers.fromRequestToGrpc(req),
                    withAuthMetadata(ctx),
                )
                .pipe(
                    map(({ data }) =>

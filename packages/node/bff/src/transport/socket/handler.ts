...
import { generateCorrelationId } from '../../utils/correlationId.ts';
import { defaultLogger } from '../../utils/logger.ts';
import { groupByWithCleanup } from '../../utils/rx/groupByWithCleanup.ts';
...
export class SocketHandler {
    private logger = defaultLogger.child({ actor: EActorName.Socket });
    ...
    private handleSocketError(session: SocketSession, error: Error) {
        const logger = this.logger.child({ sessionId: session.id, traceId: generateTraceId() });
        logger.error({

...
export class RunsTabTableRow extends TableRow {
    ...
    checkRunsData(data: TDataTask): void {
        cy.get(ERunsTabSelectors.RunsTab).within(() => {
            ...
            data.runSpeeds.forEach((runSpeed) => this.speedRowText.checkContain(runSpeed));
            data.runIds.forEach((runId) => this.idRunRowText.checkContain(runId));
            this.progressRowText.checkContain(data.progress);
            data.startTimes.forEach((startTime) => this.startTimeRowText.checkContain(startTime));

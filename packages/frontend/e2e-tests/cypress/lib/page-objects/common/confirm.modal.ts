...
export class ConfirmModal {
    ...
    readonly removeTabsCoinFilterButton = new Input(
        EModalSelectors.RemoveTabsCoinFilterButton,
        false,
    );
    ...
    setTabsCoinFilter(nameTabs: string[]) {
        this.tabsCoinFilterInput.lastClick();
        nameTabs.forEach((nameIndicator) => {
            customWait(0.3);
            this.tabsCoinFilterInput.selectsText(nameIndicator);
        });
        customWait(0.3);

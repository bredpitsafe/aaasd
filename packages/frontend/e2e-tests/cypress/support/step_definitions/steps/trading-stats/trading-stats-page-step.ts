...
Given(
    ...
    (sinceDate: string, tillDate: string) => {
        tradingStatsPage.tradingSinceDateInput.clearAndType(sinceDate);
        tradingStatsPage.tradingTillDateInput.clearAndType(tillDate);
...
Given(`user selects the first row in the "Trades" table`, () => {
    ...
    tradesTableRow.tableRowText.firstClick();
    customWait(1);
});

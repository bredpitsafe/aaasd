...
Given(
    ...
    (name: string, tableName: string) => {
        ...
        switch (tableName) {
            case 'Robots':
                checkUrlInclude('robot/' + ERobots.HerodotusMulti);
                break;
            case 'MD Gates':
                checkUrlInclude('gate/' + EGates.BinanceSwap);
                break;
            case 'Servers':
    ...
    (componentName: string, tableName: string) => {
        switch (tableName) {
            case 'Robots':
                ...
                tradingServersManagerPage.mainTitleText.checkContain(componentName);
...
Given(
    `user goes on the {string} tab of the {string} {string}`,

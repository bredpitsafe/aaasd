...
import { customWait, waitOneHeartbeat } from '../../../../../../lib/web-socket/server';
...
Given(`user types {string} name in the "Virtual Accounts" input`, (accountName: string) => {
    ...
    tableFilter.virtualAccountInput.typeAndEnter(accountName);
    tableHeader.checkVisibleRowsTable();
});
...
Given(/user sees the inputted "User Name" in the table/, () => {
    ...
    cy.get('@userVirtualName').then((object) => {
        ...
        tableFilter.virtualAccountInput.typeAndEnter(userVirtualName);
...
Given(`user selects "AutotestRealAccountName" in the "Real Accounts" section`, () => {
    cy.get('@userName').then((object) => {
        const userVirtualName = object as unknown as string;
        virtualAccountsNewAccountModal.realAccountsSelect.selectByText(userVirtualName);
    });
});
...
Given(
    ...
    () => {
        ...
        cy.get('@userNameSecond').then((object) => {
            ...
            virtualAccountsNewAccountModal.realAccountsSelect.selectByText(userNameSecond);
...
Given(`user selects two "AutotestRealAccountName" in the "Real Accounts" section`, () => {
    cy.get('@userName').then((object) => {
        const userName = object as unknown as string;
        virtualAccountsNewAccountModal.realAccountsSelect.selectByText(userName);
        cy.get('@userNameSecond').then((object) => {
            const userNameSecond = object as unknown as string;
...
Given(`user opens the "Virtual Accounts" and "Real Accounts" modal and edits name`, () => {
    ...
    cy.get('@userVirtualName').then((object) => {
        ...
        realAccountsNewAccountModal.nameInput.clearAndType(userVirtualName + random);

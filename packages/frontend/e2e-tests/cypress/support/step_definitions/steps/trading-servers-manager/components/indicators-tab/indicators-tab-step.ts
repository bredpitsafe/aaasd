...
Given(`user clicks on the "Case Sensitive" switch button`, () => {
    tableFilter.switchButton.checkVisible();
    tableFilter.switchButton.click();
    customWait(1);
});

Given(`user clicks on the {string} time button`, (time: string) => {
    indicatorsTab.checkElementsExists();
    tableHeader.checkVisibleRowsTable();
    indicatorsTab.updateTimeButton.selectByText(time);
    tableHeader.checkVisibleRowsTable();
});
...
Given(`user sets "Update Time" in the "Data" input`, () => {
    indicatorsTableRow.getActualTimeIndicator();

    cy.get('@indicatorTime').then((object) => {
        const dashboardTime = object as unknown as string;
        tableFilter.dataInput.typeAndEnter(dashboardTime);
        tableHeader.checkVisibleRowsTable();
        customWait(1);
    });
});
...
Given(`user sees a new "Update Time" in the table`, () => {
    indicatorsTableRow.getNewActualTimeIndicator();

    cy.get('@indicatorTime').then((object) => {
        const dashboardTime = object as unknown as string;
        cy.get('@indicatorNewTime').then((object) => {

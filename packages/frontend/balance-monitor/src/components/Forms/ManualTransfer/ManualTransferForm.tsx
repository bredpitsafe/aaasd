...
export const ManualTransferForm = memo(
    ({
        ...
        const { values, errors, isValid, validateForm, setTouched, setValues } = formik;
        ...
        const { accountBalanceDisplay, accountPercentDisplay } = useAvailableAmounts(
            coinInfo,
            isSyncDesc(convertRatesDescriptor) ? convertRatesDescriptor.value : undefined,
            values,
        );

        const amountTitle = useMemo(() => {
            if (!isSyncDesc(convertRatesDescriptor) || isNil(values.amount) || isNil(values.coin)) {
                return 'Amount';
            }

            const amountUsd = formatQuoteAmount(
                values.amount,
                convertRatesDescriptor.value.get(values.coin),
                CONVERSION_DIGITS,
            );
        ...
        }, [convertRatesDescriptor, values.amount, values.coin]);
        ...
        return (
            ...
            >
                ...
                <FormikForm.Item className={cnInputField} name="amount" label={amountTitle}>
                    <FormikInputNumber

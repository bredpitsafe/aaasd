...
import { ModuleSubscribeToLimitingTransferRulesHandler } from '@frontend/common/src/modules/handlers/balanceMonitor/ModuleSubscribeToLimitingTransferRulesHandler';
...
const createFail = FailFactory('AmountLimitsRulesSubscription');
const descFactory = ValueDescriptorFactory<
    TAmountLimitsRuleInfo[],
    TFail<'[AmountLimitsRulesSubscription]: UNKNOWN'>
>();

export type TAmountLimitsRulesDescriptor = ExtractValueDescriptor<typeof descFactory>;

export const ModuleAmountLimitsRules = ModuleFactory((ctx: TContextRef) => {
    let subscribeTraceId: TraceId | undefined = undefined;

    const getAmountLimitsRules$ = dedobs(
        (traceId: TraceId): Observable<TAmountLimitsRulesDescriptor> => {
            const subscribeToToLimitingTransferRulesHandler =
                ModuleSubscribeToLimitingTransferRulesHandler(ctx);
            ...
            return currentSocketUrl$.pipe(
                ...
                distinctUntilChanged(),
                switchMap((url) =>
                    subscribeToToLimitingTransferRulesHandler(url, {}, { traceId }).pipe(
                        ...
                        scan((acc, payloads) => {
                            ...
                            return acc;
                        }, new UpdatableUnifierWithCompositeHash<TAmountLimitsRuleInfo, 'id'>('id')),
        ...
        },
        {
            normalize: ([traceId]) => {
                ...
                if (subscribeTraceId !== undefined && subscribeTraceId !== traceId) {
                    logger.info(
                        `ModuleGetAmountLimitsRules: traceId ${traceId} reuse observable with traceId ${subscribeTraceId}`,
                    );
                }

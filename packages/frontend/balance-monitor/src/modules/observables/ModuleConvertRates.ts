...
import { ModuleSubscribeToConvertRatesHandle } from '@frontend/common/src/modules/handlers/balanceMonitor/ModuleSubscribeToConvertRatesHandle';
...
const createFail = FailFactory('ConvertRates');
const descFactory = ValueDescriptorFactory<
    ReadonlyMap<TCoinId, TCoinConvertRate>,
    TFail<'[ConvertRates]: UNKNOWN'>
>();

export type TConvertRatesDescriptor = ExtractValueDescriptor<typeof descFactory>;

export const ModuleConvertRates = ModuleFactory((ctx: TContextRef) => {
    let subscribeTraceId: TraceId | undefined = undefined;

    const getConvertRates$ = dedobs(
        (traceId: TraceId): Observable<TConvertRatesDescriptor> => {
            const subscribeToConvertRatesHandle = ModuleSubscribeToConvertRatesHandle(ctx);
            ...
            return currentSocketUrl$.pipe(
                ...
                distinctUntilChanged(),
                switchMap((url) =>
                    subscribeToConvertRatesHandle(url, {}, { traceId }).pipe(
                        ...
                        map((cache) =>
                            descFactory.sync(
                                new Map(
                                    cache.getMap() as unknown as ReadonlyMap<
                                        TCoinId,
                                        TCoinConvertRate
                                    >,
                                ),
        ...
        },
        {
            normalize: ([traceId]) => {
                ...
                if (subscribeTraceId !== undefined && subscribeTraceId !== traceId) {
                    logger.info(
                        `ModuleGetConvertRates: traceId ${traceId} reuse observable with traceId ${subscribeTraceId}`,
                    );
                }

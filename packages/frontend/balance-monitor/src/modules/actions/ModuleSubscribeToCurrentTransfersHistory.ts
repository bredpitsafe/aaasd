...
import type { TTransferHistoryItem } from '@frontend/common/src/types/domain/balanceMonitor/defs.ts';
...
import { ModuleSubscribeToTransferHistory } from './ModuleSubscribeToTransferHistory.ts';

export type TTransfersHistoryDescriptor = TValueDescriptor2<TTransferHistoryItem[]>;

export const ModuleSubscribeToCurrentTransfersHistory = createObservableProcedure(
    (ctx: TContextRef) => {
        const subscribeToTransferHistory = ModuleSubscribeToTransferHistory(ctx);
        const { currentSocketUrl$ } = ModuleSocketPage(ctx);

        return (_, options): Observable<TTransfersHistoryDescriptor> => {
            return currentSocketUrl$.pipe(
                filter((url): url is TSocketURL => url !== undefined),
                switchMap((target) =>
                    subscribeToTransferHistory(
                        { target },
                        ...
                        scanValueDescriptor(
                            (
                                acc:
                                    | undefined
                                    | TValueDescriptor2<
                                          UnifierWithCompositeHash<TTransferHistoryItem>
                                      >,
                                { value: events },
                            ) => {
                                const cache =

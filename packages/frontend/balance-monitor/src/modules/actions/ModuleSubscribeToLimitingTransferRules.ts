...
const descriptor = createRemoteProcedureDescriptor<TEmptySendBody, TReceiveBodyRaw>()(
    EPlatformSocketRemoteProcedureName.SubscribeToLimitingTransferRules,
    ERemoteProcedureType.Subscribe,
);
export const ModuleSubscribeToLimitingTransferRules = createRemoteProcedureCall(descriptor)({
    getPipe: () =>
        pipe(
            ...
            mapValueDescriptor((vd): TValueDescriptor2<TSubscriptionEvent<TReceiveBody>> => {
                if (
                    ...
                    vd.value.payload.type !== 'LimitingTransferRuleApplied'
                ) {

...
import { Graphics, Renderer, utils } from 'pixi.js';

import { TPartInterval } from '../../../lib/Parts/def';
import { getState } from '../../Charter/methods';
import { IContext } from '../../types';

const COLOR = utils.string2hex(blue[5]);
const HEIGHT = 4;

export class ChartLoadableIntervals extends Graphics {
    ...
    render(renderer: Renderer): void {
        const loadableIntervals = this.ctx.partsController.getAllLoadableIntervals();

        if (loadableIntervals && loadableIntervals.length > 0) {
            this.renderLine(loadableIntervals);

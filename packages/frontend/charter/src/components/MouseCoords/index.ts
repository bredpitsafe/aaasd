...
import { BitmapText, Container, Graphics, Renderer } from 'pixi.js';

import { getState, getZonedTime } from '../../Charter/methods';
import { defaultFontName } from '../../font';
...
export class MouseCoords extends Container {
    ...
    private readonly texts = {
        textX: new BitmapText('0', { fontName: defaultFontName }),
        textYLeft: new BitmapText('0', { fontName: defaultFontName }),
        textYRight: new BitmapText('0', { fontName: defaultFontName }),
    };
    ...
    render(renderer: Renderer): void {
        ...
        this.renderXAxisText(mousePoint);
        this.renderYAxisLeftText(mousePoint);
        this.renderYAxisRightText(mousePoint);

        super.render(renderer);
    }

    private updateComponentVisibility(mouseHovered: boolean, pseudoMouseHovered: boolean): void {
        const { showPseudoHorizontalCrosshair } = getState(this.ctx);

        this.texts.textX.visible = mouseHovered;
        ...
        this.graphics.backgroundX.visible = mouseHovered;
    }
    ...
    private renderXAxisText(mousePoint: TPoint): void {
        ...
        const width = textMeasure.width + TEXT_GAP * 2 + 10;
        const height = textMeasure.height + TEXT_GAP * 2;
        const x = Math.min(
            Math.max(mousePoint.x - width / 2, graphicsGap.l),
            screenWidth - width - graphicsGap.r,
        );
        const y = screenHeight - graphicsGap.b + LINE_WIDTH;
    ...
    private renderYAxisLeftText(mousePoint: TPoint): void {
        ...
        const width = textMeasure.width + TEXT_GAP * 2 + 6;
        const height = textMeasure.height + TEXT_GAP * 2;
        const x = graphicsGap.l + LINE_WIDTH;
        const y = Math.max(mousePoint.y - height, graphicsGap.t);
    ...
    private renderYAxisRightText(mousePoint: TPoint): void {
        ...
        const width = textMeasure.width + TEXT_GAP * 2 + 6;
        const height = textMeasure.height + TEXT_GAP * 2;
        const x = screenWidth - graphicsGap.r - width - 2 * LINE_WIDTH;
        const y = Math.max(mousePoint.y - height, graphicsGap.t);

...
import { TChartLegendClickModifiers } from '../../ChartLegends/view';

export function useChartsVisibility(charts: TChartProps[]) {
    ...
    const toggleChartVisibility = useFunction(
        (chart: TChartProps, modifiers: TChartLegendClickModifiers) => {
...
function getHiddenChartsIds(
    ...
    toggleChart: TChartProps,
    modifiers: TChartLegendClickModifiers,
) {
    const { id } = toggleChart;
    ...
    // Switch all charts with the same group to the state of the clicked chart.
        ...
        const groupChartIds = charts.map((chart) => chart.id);

...
import { iso2nanoseconds, nanoseconds2iso } from '../../utils/time';
...
export function TableLabelIndicatorsDashboard(props: TTableLabelIndicatorsDashboardProps) {
    ...
    return (
        <>
            <a
                {...createTestProps(EDashboardsTabSelectors.DashboardLinkButton)}
                href={url}
                target="_blank"
                rel="noreferrer"
                onClick={cbClick}
            >
                <TableLabelButton
                    {...createTestProps(ETableTabFilterSelectors.DashboardButton)}
                    icon={icon}
                    title={
                        selected

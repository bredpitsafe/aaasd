        ...
        const closeChannel = supportChannel(envelope, (channel) => {
            ...
            return (reason) => {
                ...
                if (reason === ChannelCloseReason.LoseChannel) {
                    ...
                    loggerWebactor.error('Lose channel');
                }
            };
        });
    });
...
>(
        ...
        const closeChannels = openChannel(createEnvelope(requestBox.type, body), (channel) => {
            ...
            return (reason) => {
                ...
                if (reason === ChannelCloseReason.LoseChannel) {
                    subscriber.error(
                        new WebactorError(
                            'Lose channel, for further correct work you should restart your application',
                        ),
                    );
                }
            };
    ...
    }).pipe(tapError((err: Error) => loggerWebactor.error('An error occurred', err)));
}

...
type TTradingStatsFilterProps = Pick<
    TWithLiveUpdates,
    'error' | 'live' | 'toggleLive' | 'updateTime'
> & {
    assets: TAsset[] | null | undefined;
    exchanges: TExchange[];
    instruments: TInstrument[] | null | undefined;
...
const dailyStatsSchemaShape = {
    date: Yup.string().test(
        'is-valid-date',
        `Provide date in ${EDateTimeFormats.Date} format`,
        (value) => dayjs(value, EDateTimeFormats.Date).isValid(),
    ),
...
type TTradingStatsFilterRendererProps = TWithFormik<TTradingStatsFilter> &
    Pick<TWithLiveUpdates, 'error' | 'live' | 'toggleLive' | 'updateTime'> & {
        timeZone: TimeZone;
        isMonthlyStats: boolean;
        strategies: TStrategyName[];
        assets: TAsset[] | null | undefined;
...
function TradingStatsFilterRenderer(props: TTradingStatsFilterRendererProps): ReactElement | null {
    ...
    /* Rerun form validation upon receiving new external error value to show it in the form. */
    useEffect(() => {
        formik.validateForm();
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [error]);

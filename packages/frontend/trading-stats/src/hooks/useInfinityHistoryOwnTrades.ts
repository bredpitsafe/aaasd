...
export function useInfinityHistoryOwnTrades(filters: TDailyStatsFilter, timeZone: TimeZone) {
    ...
    const fetch$ = useCallback(
        (params: TInfinityHistoryItemsFetchProps<TOwnTrade, TOwnTradeFilter>) => {
            ...
            return isNil(url)
                ? EMPTY
                : fetchOwnTrades$(url, params).pipe(tap(setCurrentUpdateTime));
        },
        [url, fetchOwnTrades$, setCurrentUpdateTime],
    );
            ...
            ? () => {
                  ...
                  return timer(300).pipe(
                      exhaustMap(() =>
                          isNil(url)
                              ? EMPTY
                              : subscribeToOwnTradesUpdates(url, filters, timeZone, options),
                      ),

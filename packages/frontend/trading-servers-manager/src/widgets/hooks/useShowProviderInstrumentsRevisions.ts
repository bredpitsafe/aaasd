...
export function useShowProviderInstrumentsRevisions(): (
    ...
    return useFunction(async (instruments: (TInstrument | TInstrumentDynamicData)[]) => {
        ...
        await setParams({ providerInstrumentsRevs: instrumentIds });

        upsertTab(EDefaultLayoutComponents.ProviderInstrumentRevisions, {

...
export const ModuleSubscribeToCurrentBacktestingRunId = ModuleFactory((ctx) => {
    const { state$ } = ModuleBacktestingRouter(ctx);

    return dedobs(
        (): Observable<undefined | TBacktestingRun['btRunNo']> => {
            return state$.pipe(
                map((v) => extractRouterParam(v.route, EBacktestingSearchParams.BacktestingRunId)),
                distinctUntilChanged<TBacktestingRun['btRunNo'] | undefined>(),
            );
        },

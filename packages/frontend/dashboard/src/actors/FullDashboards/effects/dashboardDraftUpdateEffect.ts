...
import { TWithTraceId } from '@frontend/common/src/handlers/def.ts';
import { TStorageDashboardId } from '@frontend/common/src/types/domain/dashboardsStorage.ts';
...
import { seconds2milliseconds, toSeconds } from '@frontend/common/src/utils/time';
import {
    convertErrToGrpcFail,
    convertGrpcFailToGrpcError,
} from '@frontend/common/src/utils/ValueDescriptor/Fails.ts';
import { TUnsyncedValueDescriptor } from '@frontend/common/src/utils/ValueDescriptor/types.ts';
import {
...
import { catchError, filter, groupBy, mergeMap, share, switchMap } from 'rxjs/operators';
...
const BRANCH_LIVE = seconds2milliseconds(toSeconds(60));
...
export function dashboardDraftUpdateEffect(
    ...
    const updateProcessor$ = updatesQueue.pipe(
        groupBy(({ fullDashboard }) => fullDashboard.storageDashboardItemKey.storageId, {
            duration: debounceTime(BRANCH_LIVE),
        }),
        mergeMap((group) =>
            group.pipe(
    ...
    register(updateDashboardDraftProcedureDescriptor, (fullDashboard, { traceId }) => {
        ...
        return new Observable((subscriber) => {
            const isSameDashboard = (id: null | TStorageDashboardId) =>
                id === fullDashboard.storageDashboardItemKey.storageId;
            const updateProcessorSub = updateProcessor$

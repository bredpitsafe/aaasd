...
import { ModuleResetDashboardDraft } from '../dashboards/resetDashboardDraft.ts';
...
function InnerModalFullDashboardEditor({
    ...
    const [subscribeToDashboard, updateFullDashboardFromEditor] = useModules(
        ModuleSubscribeToDashboard,
        ModuleUpdateFullDashboardFromEditor,
    );
    const dashboardDescriptor = useNotifiedValueDescriptorObservable(
        subscribeToDashboard(dashboardItemKey, { traceId }),
    );

    const [resetDashboardDraft] = useNotifiedObservableFunction(
        useModule(ModuleResetDashboardDraft),
    );
    ...
    const handleConfigDiscard = useFunction(async () => {
        assert(isSyncedValueDescriptor(dashboardDescriptor), 'dashboard is not synced');
        return resetDashboardDraft(getDashboardItemKeyFromDashboard(dashboardDescriptor.value), {
            traceId,
        });
    });
